...
Open **Automate → Create**. The demo opens the only template, **Auto-hide negative comments**, in the builder. The agent suggests the setup and asks which mock Facebook or Instagram pages to watch before editing the draft. Preview shows simulated comment matches; the Admin analytics page contains placeholder KPIs. `pnpm typecheck && pnpm test && pnpm build` should still pass when you finish.

## Original challenge references

The screenshots below document the original challenge baseline. The current demo is focused on the comment-hiding flow, so the other prompt examples are references rather than flows available from the UI.
